...
#### 実装上の追加: メタ更新を直列化する単一ロック(`sbi->lock`)

設計は「単一スレッド前提」だが、**カーネルの writeback(flusher kthread)は FS とは独立に
並行動作**し、`->write_inode` を任意のタイミングで呼ぶ。とくに `llfs_iomap_begin` が
「block bitmap 確保 → `inodei->block[sector]` 代入」を行う途中(`sb_bread` で sleep し得る)に
flusher の `write_inode` が割り込むと、**確保前の `block[]=0` を itable へ直列化**してしまい、
bitmap だけ確保済み・itable 未反映のままコミットされて再マウント後に**リークブロック**になる
(SMP だけでなく単一 CPU でも `sb_bread` の sleep 契機で発生する)。

これを防ぐため `struct llfs_sb_info` に `struct mutex lock` を1つ持ち、次を相互排他にする:

| 箇所 | ロック区間 |
|---|---|
| `llfs_iomap_begin`(IOMAP_WRITE) | `block[]` 参照〜`llfs_alloc_block`〜`block[]` 代入〜`mark_inode_dirty` |
| `llfs_write_inode` | itable 読み〜直列化〜`llfs_txn_log` |
| `llfs_create` | `llfs_make_inode`(inode bitmap 確保)〜`llfs_add_dirent`〜`mark_inode_dirty` |
| `llfs_txn_commit`(公開版) | commit 全体 |

`llfs_txn_log` は「呼び出し側がロック保持」を前提とし、`llfs_txn_commit` は内部の
`__llfs_txn_commit`(ロック保持前提)を lock で包む公開ラッパとする(txn-full 再入は
内部版を直接呼ぶ)。ロック保持側が `write_inode` を誘発する経路は無いため再入デッドロックは無い。
...
- ~~単一スレッド前提のロックレス実装~~ → flusher と並行するため `sbi->lock` で直列化(§8.6 追記)。
  並行マウント操作は依然対象外。
- §7 の「`blocks` カウント不統一」「`bno << SHIFT` の 32bit 演算」等の既存沼は本設計と直交。
  ジャーナル化の前後で別途修正してよい。

### 8.13 実装完了状況と検証(QEMU + fsck.py)

**Phase 0–2 を実装済み**(§8.10)。`journal.c` / `journal.h` を追加し、`Makefile` に `journal.o` を追加。
`mkfs.py` / `fsck.py` を新レイアウト(journal 領域 block 5〜36, root-data=37)に更新。

#### 検証で判明し修正した致命バグ(ジャーナルとは独立)

クラッシュ整合性テスト(`meta-loop` で多数のファイルを作り、ランダムなタイミングで QEMU を
`kill -9` → 再マウントで replay → `fsck.py`)を回したところ、**長く churn したラウンドだけ**
大量の「リークブロック」で `INCONSISTENT` になった。原因はジャーナルではなく:

- **`llfs_get_first_free_inode` が itable 容量(= `block_size/inode_size` = 64)を超える
  inode 番号を返していた**。inode bitmap は 32768 ビットあるため、63 個目以降のファイル作成で
  `find_first_zero_bit` が 64,65,… を返し、その `write_inode` が `itable->table[64]` =
  **itable ブロック(4096B=64 スロット)の外**、すなわち隣接する inode/block bitmap ブロックへ
  書き込んでメタデータを破壊していた。`fsck` からは「誰も参照しない確保済みブロック」に見える。
- 併せて `llfs_make_inode` が確保失敗時に `new_inode` した VFS inode を `iput` せずリーク、
  `llfs_create` が `if(!inode)` で `ERR_PTR` を素通ししていた(§7-4 の沼)も修正:
  `find_first_zero_bit` を `max_inodes` で打ち切り `>= max_inodes` を満杯と判定、
  `make_inode` は失敗時 `iput` + `ERR_PTR(-ENOSPC)`、`create` は `IS_ERR` 判定 + `add_dirent`
  の戻り値チェックに修正。

→ これで 63 個目の作成は `-ENOSPC` で正しく失敗し、`meta-loop` は 62 ファイルで頭打ちになる。

#### 検証結果

`mkfs` → churn → `kill -9`(電源断模擬, 1.5〜11s のランダム点)→ 再マウント(replay)→
clean umount → `fsck.py` を**多数ラウンド**実施し、リカバリ種別が **empty / torn-discard /
replay のいずれでも `CONSISTENT`** になることを確認(replay 経路・torn 破棄経路の両方を踏むことを
ログで確認済み)。リーク/クロスリンク/ダングリングはいずれも検出されない。

> 留意: QEMU の `kill -9` はホストページキャッシュを失わないため「発行済み書き込みは全て残る」
> モデルに相当する(デバイスキャッシュ喪失までは模擬しない)。それでも WAL は「書き込み列の
> 任意の前置(prefix)で一貫」を保証するため、本テストは replay/torn 破棄の正しさを十分に検証できる。

#### 検証スクリプト(リポジトリ直下)

- `jloop.sh N` — N ラウンドの「mkfs → churn → ランダム kill → remount(recover) → umount → fsck」。
- `jone.sh`(`KILL=`/`SMP=` 環境変数)— 1 回だけ実行し、不整合なら `/tmp/bad.img`(replay 後)/
  `/tmp/crash.img`(replay 前)を保存して `modules/llfs/inspect.py` で詳細ダンプ。
- `modules/llfs/inspect.py <img>` — inode 別の bitmap/mode/size/blocks/dirent/block[] と
  リーク/クロスリンク/未マークを一覧する詳細チェッカ。
