...
def write_super(f):
    ...
    f.write(struct.pack("<IIIIIH",
                        MAGIC, BLOCK_SIZE,
                        ITABLE_BLOCK, IBITMAP_BLOCK, BBITMAP_BLOCK,
                        INODE_SIZE))
...
def write_inode(f, ino, mode, uid, size, blocks, block_ptrs):
    ...
    raw = struct.pack("<HHII", mode, uid, size, blocks) \
        + struct.pack("<%dI" % N_BLOCKS, *arr)
...
def write_root_dir(f):
    ...
    dotdot = make_dirent(ROOT_INO, "..", FT_DIR, BLOCK_SIZE - (DIRENT_HEADER + 4))
